<template>
    <section class="pagination pager" v-if="count>1">
        <ul class="pages pgTxCenter">
            <li class="pgNext pgpre" :class="{'pgEmpty':index==1}" @click="prev"><i></i></li>
            <li class="page-number" v-for="value in pageList"
                ...
                @click="go(value)"
            >{{value}}</li>
            <li class="pgNext pgAfter" :class="{'pgEmpty':index==count}" @click="next"><i></i></li>
        </ul>
        <div class="pgJump" v-if="count>=14">
            <span>跳转到：</span>
            <input type="text" class="pgJumpNumber" v-model="pageNumber">
            <span class="pgText">页</span>
            <input type="button" value="确定" class="pgJumpSubmit" @click="pageJump">
        </div>
    </section>
</template>
